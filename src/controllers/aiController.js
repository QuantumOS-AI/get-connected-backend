...
const N8N_REPLY_WEBHOOK_URL =
  process.env.N8N_REPLY_WEBHOOK_URL || "YOUR_N8N_REPLY_WEBHOOK_URL_HERE";
...
exports.handleWebhookMessage = async (req, res, next) => {
  try {
    ...
    if (!message || !contactId || !userId) {
      ...
      console.error(
        "Webhook Error: Missing required fields (message, contactId, userId)",
        req.body
      );
      ...
      return res.status(200).json({
        success: true,
        message: "Received, but missing required fields.",
      });
    }
    ...
    if (!userExists) {
      ...
      return res
        .status(200)
        .json({ success: true, message: "Received, but user not found." });
    }
    ...
    if (!contactExists) {
      console.error(
        `Webhook Error: Contact with ID ${contactId} not found or doesn't belong to user ${userId}.`
      );
      return res.status(200).json({
        success: true,
        message: "Received, but contact not found or invalid.",
      });
    }
    ...
    if (estimateId) {
      const estimateExists = await prisma.estimate.findFirst({
        where: { id: estimateId, createdBy: userId, clientId: contactId },
      });
      if (!estimateExists) {
        console.error(
          `Webhook Error: Estimate with ID ${estimateId} not found or doesn't belong to contact ${contactId} / user ${userId}.`
        );
        return res.status(200).json({
          success: true,
          message: "Received, but estimate not found or invalid.",
        });
      }
    }
    ...
    // const { broadcast } = require('../server'); // Removed: broadcast will be accessed via req.app.get('broadcast')
    const broadcast = req.app.get("broadcast");
    if (broadcast) {
      broadcast({ type: "newMessage", data: aiMessage });
    } else {
      console.error("Broadcast function not available on req.app");
    }

    res.status(200).json({
      success: true,
      message: "AI message received and saved.",
      data: aiMessage,
    });
  } catch (error) {
    ...
    res.status(200).json({
      success: true,
      message: "Received, but internal server error occurred.",
    });
...
exports.getConversation = async (req, res, next) => {
  try {
    ...
    if (contactId) {
      ...
      return next(
        new AppError("Please provide either a contactId or estimateId", 400)
      );
    }
    ...
    if (estimateId) {
      const estimate = await prisma.estimate.findFirst({
        where: { id: estimateId, createdBy: userId },
      });
      if (!estimate)
        return next(new AppError("Estimate not found or access denied", 404));
      // Optionally ensure estimate belongs to the contact if both are somehow provided
      if (contactId && estimate.clientId !== contactId) {
        return next(
          new AppError("Estimate does not belong to the specified contact", 400)
        );
      }
  ...
  try {
    ...
    if (estimateId) {
      ...
      if (!estimate) {
        return next(
          new AppError(
            "Estimate not found or does not belong to this contact",
            404
          )
        );
      }
    ...
    if (N8N_REPLY_WEBHOOK_URL === "YOUR_N8N_REPLY_WEBHOOK_URL_HERE") {
      console.warn(
        "N8N_REPLY_WEBHOOK_URL is not configured. Skipping AI response trigger."
      );
      // Optionally return here or proceed without triggering AI
      return res.status(201).json({
        success: true,
        message:
          "User message saved. AI response trigger skipped (Webhook URL not configured).",
        data: userMessage,
      });
    }
    ...
    try {
      ...
      console.error(
        "Error triggering N8N reply webhook:",
      ...
      return next(
        new AppError("Message saved, but failed to trigger AI response.", 500)
      );
    }
  ...
  try {
    ...
    for (const convo of distinctConversations) {
      ...
      if (convo.contactId && convo.estimateId) {
        ...
        console.warn(
          `Found AiMessage with estimateId ${convo.estimateId} but no contactId for user ${userId}. Skipping.`
        );
